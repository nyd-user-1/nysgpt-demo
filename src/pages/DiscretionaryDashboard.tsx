...
import {
  useDiscretionaryDashboard,
  formatCompact,
  type DiscretionaryGroupRow,
  type DiscretionaryDrillRow,
} from '@/hooks/useDiscretionaryDashboard';
import {
  ResponsiveContainer,
  BarChart,
  Bar,
...
export default function DiscretionaryDashboard() {
  ...
  const { isLoading, error, byAgency, grandTotal, totalItems, getDrillDown } = useDiscretionaryDashboard();
  ...
  const openChat = (context?: string, drillName?: string) => {
    const prompt = drillName
      ? `Tell me about NYS discretionary grants for "${drillName}". ${context || ''}`
      : `Tell me about NYS discretionary grants. ${context || ''}`;
      ...
      <InsetPanel>
            ...
            {!isLoading && !error && (
              <div className="text-right flex-shrink-0">
                ...
                <span className="text-sm text-muted-foreground">
                  {selectedRow ? `${((byAgency.find(r => r.name === selectedRow)?.totalAmount ?? 0) / grandTotal * 100).toFixed(1)}% of total` : 'Discretionary Grants'}
                </span>
              </div>
            )}
            <MobileNYSgpt />
            ...
            <div className="h-48 md:h-56 px-4 pb-2">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} margin={{ top: 4, right: 8, bottom: 16, left: 8 }}>
                  <Bar dataKey="amount" fill="hsl(280 67% 55%)" radius={[2, 2, 0, 0]} animationDuration={500} />
                  ...
                  <RechartsTooltip
                    ...
                    formatter={(value: number) => [formatCompact(value), 'Grants']}
        ...
        <div className="flex-1 overflow-y-auto">
          {isLoading ? (
            ...
            <div className="text-center py-12 text-destructive">Error loading discretionary data</div>
          ) : (
              ...
              {byAgency.map(row => (
                <DiscretionaryGroupRowItem
                  ...
                  onChatClick={() => {
                    ...
                    const ctx = [`Agency: ${row.name}`, `Total Grants: ${formatCompact(row.totalAmount)}`, `Count: ${row.count}`, '', 'Top grantees:', ...items.slice(0, 10).map(i => `- ${i.grantee}: ${formatCompact(i.grantAmount)}${i.year ? ` (${i.year})` : ''}`)].join('\n');
                    openChat(ctx);
                  }}
                  onDrillChatClick={(item) => {
                    const ctx = [`Grantee: ${item.grantee}`, item.agencyName ? `Agency: ${item.agencyName}` : '', item.sponsor ? `Sponsor: ${item.sponsor}` : '', `Grant Amount: ${formatCompact(item.grantAmount)}`, item.year ? `Year: ${item.year}` : '', item.description ? `Description: ${item.description}` : ''].filter(Boolean).join('\n');
                    openChat(ctx, item.grantee);
                  }}
...
// ── Row Component ────────────────────────────────────────
function DiscretionaryGroupRowItem({ row, isExpanded, onToggle, onChatClick, onDrillChatClick, getDrillDown }: {
  row: DiscretionaryGroupRow;
  isExpanded: boolean;
  onToggle: () => void;
  onChatClick: () => void;
  onDrillChatClick: (item: DiscretionaryDrillRow) => void;
  getDrillDown: (name: string) => DiscretionaryDrillRow[];
}) {
  const navigate = useNavigate();
        ...
        <div className="bg-muted/10 border-t border-b">
          {items.map((item, idx) => (
            <div key={`${item.id}-${idx}`} className="group">
              <div onClick={() => navigate(`/discretionary/${item.id}`)} className="hidden md:grid grid-cols-[1fr_44px_120px_80px] gap-4 px-6 py-3 pl-14 text-sm hover:bg-muted/20 transition-colors items-center cursor-pointer">
                <div className="flex items-center gap-2 min-w-0">
                  <span className="truncate">{item.grantee}</span>
                  {item.sponsor && <span className="text-xs text-muted-foreground bg-muted px-1.5 py-0.5 rounded flex-shrink-0">{item.sponsor}</span>}
                </div>
                ...
                <span className="text-right tabular-nums">{formatCompact(item.grantAmount)}</span>
                <span className="text-right tabular-nums text-muted-foreground text-xs">{item.year || ''}</span>
              </div>
              {/* Mobile sub-row */}
              <div onClick={() => navigate(`/discretionary/${item.id}`)} className="md:hidden px-4 py-3 pl-10 cursor-pointer">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-sm truncate flex-1 min-w-0">{item.grantee}</span>
                  <span className="text-sm text-muted-foreground ml-2 whitespace-nowrap">{formatCompact(item.grantAmount)}</span>
                </div>
